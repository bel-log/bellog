...
export const SideMainSettings = (props: {
    ...
    const driverType = profile.driverType
    const profileName = profile.profileName
    const setProfileName = (name: string) => {
        profileStore.profile.profileName = name
    }

    const exportProfile = props.exportProfile
    const loadProfile = props.loadProfile

    const [tmpDriverSettings, setTmpDriverSettings] = useState(profile.driverSettings)
    const [driverModalIsOpen, setDriverModalIsOpen] = useState(false)

    function updateDriverType(newDriverType: DriverNames) {
        profileStore.profile.driverType = newDriverType
        profileStore.profile.driverSettings = buildDefaultDriverSettings(newDriverType)
    }

    function updateDriverSettings(newDriverSettings: DriverSettings) {
        profileStore.profile.driverSettings = newDriverSettings
    }
                ...
                <div className="field has-addons">
                    <div className="select">
                        <select value={driverType}
                                className={`${isDriverAllowedInWebMode(driverType) ? "" : "has-text-danger"}`}
                                ...
                                Object.values(DriverNames).map(
                                    (driver, dindex) => {
                                        if (isDriverAllowedInWebMode(driver)) {
                                            return <option className="has-text-black" key={dindex}
                                                           value={driver}>{driver}</option>
                                        } else {
                                            return <option className="has-text-danger" key={dindex}
                                                           value={driver}>{driver}</option>
                                        }
                                    }
                                )
                ...
                <div className="field is-grouped mt-4">
                    ...
                    <div className="control">
                        <div className="file is-primary">
                            <label className="file-label">
                                <input className="file-input" type="file" name="import" accept=".bll"
